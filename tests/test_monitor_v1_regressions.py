...
def test_windows_bootstrap_is_native_and_single_source():
    script = Path("bootstrap.ps1").read_text(encoding="utf-8")
...
def test_windows_orchestrator_owns_one_capture_source():
    source = Path("src/campus_ops/windows_orchestrator.py").read_text(encoding="utf-8")
...
def test_windows_capture_maps_alias_to_npcap_guid():
    source = Path("src/campus_ops/workers/windows_capture.py").read_text(encoding="utf-8")
...
def test_assets_require_real_repeated_packet_source_evidence():
    source = Path("src/campus_ops/workers/asset_engine.py").read_text(encoding="utf-8")
...
def test_flows_and_topology_are_packet_observed_only():
    flows = Path("src/campus_ops/workers/flow_engine.py").read_text(encoding="utf-8")
    topology = Path("src/campus_ops/workers/topology_engine.py").read_text(encoding="utf-8")
...
def test_console_has_no_admin_or_forensics_workspace():
    ui = Path("src/campus_ops/stable_ui.py").read_text(encoding="utf-8")
    operator = Path("src/campus_ops/stable_operator_ui.py").read_text(encoding="utf-8")
...
def test_system_diagnostics_are_windows_specific():
    source = Path("src/campus_ops/watchdog_api.py").read_text(encoding="utf-8")
...
def test_metadata_history_is_bounded_and_payload_free():
    source = Path("src/campus_ops/workers/evidence_store.py").read_text(encoding="utf-8")
...
def test_windows_service_is_single_instance_and_auto_recovers():
    source = Path("scripts/install_windows_service.ps1").read_text(encoding="utf-8")
...
def test_windows_diagnostics_script_exists():
    source = Path("scripts/diagnose_windows.ps1").read_text(encoding="utf-8")
    for required in (
        "Get-NetAdapter",
...
def test_dependency_constraints_do_not_reintroduce_invalid_websockets_pin():
    constraints = Path("requirements.lock").read_text(encoding="utf-8")
...
def test_unmeasured_interface_rates_are_not_rendered_as_zero():
    ui = Path("src/campus_ops/ui/index.html").read_text(encoding="utf-8")
    telemetry = Path("src/campus_ops/workers/telemetry.py").read_text(encoding="utf-8")
