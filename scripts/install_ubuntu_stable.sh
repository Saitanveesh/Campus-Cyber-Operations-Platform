...
echo '[campus-ops] Installing stable single-source monitor.'
apt-get update
...
# A stable install never reuses a previous virtualenv. This prevents old packages,
# entry points and dependency pins from contaminating the current runtime.
...
# Purge files produced by the former multi-sensor/agent runtime. Current MON does not
# read them, and leaving them behind makes field diagnostics ambiguous.
rm -f \
...
# Disable and remove service definitions from the old parallel sensor pipeline. The
# disable step is intentionally retained for upgrades from older MON installations.
for unit in \
...
find /etc/systemd/system -maxdepth 2 -type l \
    \( -name 'campus-ops-sensor@*.service' -o -name 'campus-ops-falco.service' \) \
