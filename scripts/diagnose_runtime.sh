...
echo_section "DUMPCAP CAPABILITIES"
...
systemctl show campus-ops.service \
    ...
    -p CapabilityBoundingSet 2>&1 || true

echo_section "LIVE STATUS SUMMARY"
...
print("capture_detail:", capture.get("detail"))
...
for name,row in (s.get("workers") or {}).items():
    ...
    if state not in {"HEALTHY","STOPPED"}:
        print(f"  {name}: {state} detail={row.get('detail')} error={row.get('last_error')}")
' <<<"$payload" || true
else
    echo "live status endpoint unavailable"
fi

echo_section "SYSTEM DIAGNOSTICS"
curl -fsS --max-time 4 http://127.0.0.1:8765/api/v1/system/diagnostics 2>/dev/null | python3 -m json.tool 2>/dev/null || true
