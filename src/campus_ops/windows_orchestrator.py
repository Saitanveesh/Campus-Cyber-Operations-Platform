...
class WindowsOrchestrator:
    ...
    The orchestrator intentionally owns one network discovery path and one capture
    process. Every asset, relationship, anomaly, incident and investigation is derived
    from that TShark/Npcap packet stream or from MON's own runtime health state.
    """
    ...
    def __init__(self, settings: Settings = DEFAULT_SETTINGS) -> None:
        ...
        self.evidence_store = EvidenceStoreWorker(self.bus, self.state, self.get_session_id)
    ...
    def _derive_risk_graph(cls, live: dict[str, object]) -> dict[str, object]:
        ...
        for alert in live.get("alerts", []):
            ...
            add(
                ...
                str(payload.get("title") or "alert evidence"),
            )
    ...
    def snapshot(self) -> dict[str, object]:
        ...
        return {
            ...
            "capture_stack": "tshark+npcap",
            "capture_state_policy": "process-health-only",
            "session_control_plane": "network-events-only",
            "topology_source": "current-session-tshark-packets",
            "ip_truth_policy": "current-session-packet-evidence-only",
            "history_policy": "metadata-only-7-day-local-retention",
            "platform_contract": "native-windows-only",
