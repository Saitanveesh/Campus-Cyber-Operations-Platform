...
async function contextualBrief(view,force=false){
  const now=Date.now();
  if(!force&&view===lastBriefedView&&now-lastBriefedAt<30000)return;
  ...
  try{
    ...
    lastBriefedView=view;lastBriefedAt=Date.now();lastContextText=d.text||'';
...
function scheduleContext(view){
  ...
  contextTimer=setTimeout(()=>contextualBrief(view),80);
}
