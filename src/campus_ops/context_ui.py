...
let lastView='overview',contextTimer=null,lastContextText='',contextSerial=0;
...
function browserVoice(text){
  ...
  stopBrowserVoice();
  try{const u=new SpeechSynthesisUtterance(text);u.rate=1;u.volume=1;window.speechSynthesis.speak(u);}catch{}
}
...
function scheduleContext(view){
  stopBrowserVoice();clearTimeout(contextTimer);
  contextTimer=setTimeout(()=>contextualBrief(view),100);
}
...
function wireBriefButtons(){for(const id of ['assistantBrief','assistantBrief2']){const button=C(id);if(button)button.onclick=()=>contextualBrief(activeView(),true);}}
...
function injectIncidentPanel(){
  ...
  security.insertBefore(section,security.firstChild);C('speakIncident').onclick=()=>contextualBrief('security',true);
}
...
window.stopOperationsVoice=()=>{contextSerial++;stopBrowserVoice();};
})();
</script>
"""
