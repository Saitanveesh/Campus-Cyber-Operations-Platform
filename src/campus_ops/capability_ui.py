...
.capability-score{display:grid;grid-template-columns:190px 1fr 1fr 1fr;border:1px solid #c7c7c7;margin-bottom:8px}
...
.capability-score-note{font-size:10px;color:#555;line-height:1.5;margin:0 0 14px}
...
function injectCapabilityCenter(){
  ...
  section.innerHTML='<div class="sectionhead"><div><h2>Capability Center</h2><div class="small">Operational readiness is based only on required core capabilities. Optional integrations are shown separately as extended coverage.</div></div><button class="btn" id="capabilityRefresh">Refresh</button></div><div class="capability-score" id="capabilityScore"></div><div class="capability-score-note" id="capabilityScoreNote"></div><div class="sectionhead"><h2>Monitoring Host Security</h2><div class="note">Read-only Windows Defender, Firewall and Sysmon posture.</div></div><div class="host-security-grid" id="hostSecurityGrid"></div><div class="host-security-detail" id="hostSecurityDetail">Waiting for Windows security telemetry.</div><div class="split" style="margin-top:16px"><div><div class="label" style="margin-bottom:8px">Operational Capabilities</div><div class="capability-grid" id="capabilityGrid"></div></div><div><div class="panel"><div class="label">Remaining Upgrades</div><div id="capabilityGaps" style="margin-top:8px"></div></div><div class="panel" style="margin-top:12px"><div class="label">Read-only Diagnostics</div><div class="diagnostic-grid" id="diagnosticButtons"></div><div id="diagnosticStatus" class="small">Select a diagnostic check.</div><pre class="diagnostic-output" id="diagnosticOutput">No diagnostic has been run.</pre></div></div></div>';
...
function renderHostSecurity(){
  ...
  const d=s.defender||{},fw=s.firewall||{},sysmon=s.sysmon||[],known=Object.values(fw).filter(v=>v===true||v===false).length,enabled=Object.values(fw).filter(v=>v===true).length,total=Object.keys(fw).length,sysmonRunning=sysmon.some(x=>String(x.status||'').toLowerCase()==='running');
  const firewallState=total&&known===total?`${enabled} / ${total} ON`:total?`${enabled} / ${total} ON · ${total-known} UNKNOWN`:'UNKNOWN';
  if(grid)grid.innerHTML=[['Defender',yn(d.antivirus_enabled)],['Real-time',yn(d.realtime_enabled)],['Firewall',firewallState],['Sysmon',sysmon.length?(sysmonRunning?'RUNNING':'STOPPED'):'NOT INSTALLED']].map(x=>`<div class="host-security-cell"><div class="label">${he(x[0])}</div><strong>${he(x[1])}</strong></div>`).join('');
...
function renderCapabilities(d){
  ...
  const readiness=Number(d.operational_readiness??d.score??0),coverage=Number(d.coverage_score??0),coreReady=Number(d.core_ready||0),coreTotal=Number(d.core_total||0),optionalReady=Number(d.optional_ready||0),optionalTotal=Number(d.optional_total||0);
  const score=K('capabilityScore');if(score)score.innerHTML=[['Operational Readiness',`${readiness}%`],['Core Plane',`${coreReady} / ${coreTotal} READY`],['Extended Coverage',`${d.ready||0} / ${d.total||0} · ${coverage}%`],['Next Upgrade',d.next_gaps?.[0]?.label||'None']].map(x=>`<div><div class="label">${he(x[0])}</div><strong>${he(x[1])}</strong></div>`).join('');
  const note=K('capabilityScoreNote');if(note)note.textContent=coreReady===coreTotal?`All required core capabilities are ready. ${optionalReady} of ${optionalTotal} optional capability groups are fully available; missing optional tools do not reduce operational readiness.`:`${coreTotal-coreReady} required core capability group(s) still need attention before the platform is operationally ready.`;
  ...
  const gaps=K('capabilityGaps');if(gaps)gaps.innerHTML=(d.next_gaps||[]).map((g,i)=>`<div class="capability-gap"><b>${i+1}. ${he(g.label)}</b><div>${he(g.state)} · ${he(g.plane)} · ${he(g.priority)}</div><div>${he([...(g.missing_tools||[]),...(g.missing_workers||[])].join(' · ')||'Needs validation')}</div></div>`).join('')||'<div class="muted">No capability gaps detected.</div>';
