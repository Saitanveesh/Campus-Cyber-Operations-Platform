...
@media(max-width:950px){.capability-score{grid-template-columns:1fr 1fr}.capability-score>div:nth-child(2){border-right:0}.capability-grid{grid-template-columns:1fr}}
</style>
<script>
(()=>{
...
function injectCapabilityCenter(){
  ...
  section.innerHTML='<div class="sectionhead"><div><h2>Capability Center</h2><div class="small">Readiness across visibility, intelligence, endpoint, response and evidence planes.</div></div><button class="btn" id="capabilityRefresh">Refresh</button></div><div class="capability-score" id="capabilityScore"></div><div class="split"><div><div class="label" style="margin-bottom:8px">Operational Capabilities</div><div class="capability-grid" id="capabilityGrid"></div></div><div><div class="panel"><div class="label">Highest-value Gaps</div><div id="capabilityGaps" style="margin-top:8px"></div></div><div class="panel" style="margin-top:12px"><div class="label">Read-only Diagnostics</div><div class="diagnostic-grid" id="diagnosticButtons"></div><div id="diagnosticStatus" class="small">Select a diagnostic check.</div><pre class="diagnostic-output" id="diagnosticOutput">No diagnostic has been run.</pre></div></div></div>';
  ...
  const refresh=K('capabilityRefresh');if(refresh)refresh.onclick=loadCapabilities;
}

function renderCapabilities(d){
  capabilityData=d;injectCapabilityCenter();
...
injectCapabilityCenter();loadCapabilities();setInterval(()=>{if(document.querySelector('.tab.active')?.dataset?.view==='system')loadCapabilities();},15000);
})();
</script>
"""
