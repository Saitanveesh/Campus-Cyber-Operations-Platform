...
#topologyCanvas{background:#fff;height:690px;min-height:690px;padding:8px;overflow:hidden}
...
.topo-node rect{fill:#fff;stroke:#222;stroke-width:1;rx:4;ry:4}
...
.topo-hidden rect{fill:#f5f5f5;stroke:#aaa;stroke-dasharray:4 3}
.topo-hidden text{fill:#555;font:700 9px Arial,Helvetica,sans-serif;letter-spacing:.05em}
...
.topo-focus-summary{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:12px}
.topo-focus-summary>div{border:1px solid #ddd;padding:8px;font-size:10px}
.topo-flow-table{max-height:330px}
@keyframes topoDash{to{stroke-dashoffset:-28}}
@media(max-width:1100px){#view-topology .split{grid-template-columns:1fr}#topologyCanvas{height:640px}.topo-statbar{grid-template-columns:repeat(3,1fr)}}
@media(max-width:720px){#topologyCanvas{height:580px}.topo-statbar{grid-template-columns:1fr 1fr}.topo-mode{padding:7px 6px}}
</style>
<script>
(()=>{
...
function category(id,assets,edges){
  const r=role(assets.get(id));
  if(r.includes('SENSOR'))return 'sensor';
  if(r.includes('INFRASTRUCTURE')||r.includes('GATEWAY'))return 'infra';
  if(r.includes('PUBLIC')||r.includes('OFF_SUBNET')||r.includes('MULTICAST')||r.includes('BROADCAST')||r.includes('SPECIAL'))return 'external';
  const e=edges.find(x=>x.source===id||x.target===id),er=String(e?.source===id?e?.source_role:e?.target_role||'').toUpperCase();
  if(er.includes('SENSOR'))return 'sensor';if(er.includes('INFRASTRUCTURE')||er.includes('GATEWAY'))return 'infra';if(er.includes('PUBLIC')||er.includes('OFF_SUBNET'))return 'external';return 'local';
}
...
function chooseIds(live,assets,edges,risk){
  ...
  const caps=topoDensity==='expanded'?{external:12,infra:6,local:20,sensor:4}:{external:6,infra:3,local:9,sensor:2};const keep=new Set();
  if(topoMode==='flows'){
    const flowIds=[];for(const f of (live.flows||[]).slice().sort((a,b)=>Number(b.bps_ewma||b.packets||0)-Number(a.bps_ewma||a.packets||0)).slice(0,12)){if(f.src)flowIds.push(f.src);if(f.dst)flowIds.push(f.dst);}for(const id of flowIds)keep.add(id);
  }else if(topoMode==='risk'){
    for(const id of all.slice().sort((a,b)=>(risk[b]||0)-(risk[a]||0))){if((risk[id]||0)>0)keep.add(id);if(keep.size>=(topoDensity==='expanded'?30:16))break;}
  }
  for(const [cat,arr] of Object.entries(groups)){for(const id of arr.slice(0,caps[cat]))keep.add(id);}
  if(topoSelected){keep.add(topoSelected);const neighbors=edges.filter(e=>e.source===topoSelected||e.target===topoSelected).sort((a,b)=>Number(b.bps_ewma||b.packets||0)-Number(a.bps_ewma||a.packets||0));for(const e of neighbors.slice(0,topoDensity==='expanded'?18:10))keep.add(e.source===topoSelected?e.target:e.source);}
  return {all,visible:[...keep],scores,groups,caps};
}
function placeBand(ids,yStart,yEnd,pos,W,cols){if(!ids.length)return;const rows=Math.ceil(ids.length/cols),gapY=rows<=1?0:(yEnd-yStart)/(rows-1);ids.forEach((id,i)=>{const row=Math.floor(i/cols),count=Math.min(cols,ids.length-row*cols),col=i%cols,x=count===1?W/2:130+col*(W-260)/(count-1),y=yStart+row*gapY;pos[id]={x,y};});}
function layout(ids,assets,edges,scores){const groups={external:[],infra:[],local:[],sensor:[]};ids.forEach(id=>groups[category(id,assets,edges)].push(id));for(const arr of Object.values(groups))arr.sort((a,b)=>(scores[b]||0)-(scores[a]||0));const W=1400,H=760,pos={};placeBand(groups.external,105,175,pos,W,6);placeBand(groups.infra,295,330,pos,W,4);placeBand(groups.local,470,555,pos,W,6);placeBand(groups.sensor,690,710,pos,W,4);return {groups,pos,W,H};}
...
function hiddenSvg(picked,visibleSet){const bands={external:{y:212,label:'EXTERNAL'},infra:{y:365,label:'INFRA'},local:{y:605,label:'LOCAL'},sensor:{y:735,label:'SENSOR'}},rows=[];for(const [cat,arr] of Object.entries(picked.groups)){const hidden=arr.filter(id=>!visibleSet.has(id)).length;if(!hidden)continue;const b=bands[cat];rows.push(`<g class="topo-hidden" transform="translate(1260,${b.y})"><rect x="-75" y="-15" width="150" height="30" rx="4"></rect><text x="0" y="4" text-anchor="middle">+${hidden} MORE ${b.label}</text></g>`);}return rows.join('');}
function renderStats(live,total,visible,edges){const totalRate=edges.reduce((s,e)=>s+Number(e.bps_ewma||0),0),recent=edges.filter(e=>age(e.last_seen)<8).length,hidden=Math.max(0,total-visible.length),stats=T('topologyStats');if(stats)stats.innerHTML=[['Visible Nodes',`${visible.length} / ${total}`],['Visible Links',edges.length],['Active Flows',(live.flows||[]).length],['Recent Links',recent],['Visible Rate',bits(totalRate)],['Grouped Hidden',hidden]].map(x=>`<div class="topo-stat"><div class="label">${h(x[0])}</div><b>${h(x[1])}</b></div>`).join('');const note=T('topologyFocusNote');if(note)note.textContent=hidden?`${hidden} low-priority nodes are grouped instead of being drawn. Use Find, select a node, or Expand when you need more detail.`:'All selected nodes are visible.';}
...
function renderAdvanced(live){
  ...
  const guides=[['EXTERNAL / OFF-SUBNET',55],['GATEWAY / INFRASTRUCTURE',250],['LOCAL / VLAN',420],['SENSOR',650]].map(([name,y])=>`<text class="topo-band-label" x="35" y="${y}">${name}</text><line class="topo-band-line" x1="35" y1="${Number(y)+12}" x2="1365" y2="${Number(y)+12}"/>`).join('');
  const edgeSvg=edges.map(e=>{const a=pos[e.source],b=pos[e.target];if(!a||!b)return '';const selected=topoSelected&&(e.source===topoSelected||e.target===topoSelected),unrelated=topoSelected&&!selected,recent=age(e.last_seen)<8,value=Number(e.bps_ewma||0)||Number(e.packets||0),width=1+Math.min(3,Math.log10(1+value)/Math.max(1,Math.log10(1+maxTraffic))*3),opacity=unrelated?.035:selected?1:recent?.62:.14,label=selected?`<text class="topo-edge-label" x="${(a.x+b.x)/2}" y="${(a.y+b.y)/2-5}" text-anchor="middle">${h(appOf(e)||e.last_protocol||'traffic')} · ${bits(e.bps_ewma||0)}</text>`:'';return `<path class="topo-edge ${recent?'recent':''} ${selected?'selected':''}" d="${curve(a,b)}" style="stroke-width:${width.toFixed(2)};opacity:${opacity}" marker-end="url(#${recent||selected?'tArrowHot':'tArrow'})"><title>${h(e.source)} → ${h(e.target)} · ${h(appOf(e)||'traffic')} · ${bits(e.bps_ewma||0)}</title></path>${label}`;}).join('');
  const nodeSvg=picked.visible.map(id=>{const p=pos[id];if(!p)return '';const a=assets.get(id)||{},lab=label(id,a),selected=id===topoSelected,connected=!topoSelected||edges.some(e=>(e.source===topoSelected&&e.target===id)||(e.target===topoSelected&&e.source===id))||selected,opacity=connected?1:.22,rv=risk[id]||0;return `<g class="topo-node ${selected?'selected':''} ${rv>0?'risk':''}" data-adv-node="${h(id)}" transform="translate(${p.x},${p.y})" style="opacity:${opacity}"><rect x="-70" y="-22" width="140" height="44"></rect><text x="0" y="-4" text-anchor="middle">${h(lab.main)}</text><text class="node-sub" x="0" y="12" text-anchor="middle">${h(lab.sub.slice(0,31))}</text><title>${h(role(a))}${rv?` · risk ${rv}/100`:''}</title></g>`;}).join('');
  const counts=`<text class="topo-band-label" x="1360" y="24" text-anchor="end">${groups.external.length} EXT · ${groups.infra.length} INFRA · ${groups.local.length} LOCAL · ${groups.sensor.length} SENSOR</text>`;
  canvas.innerHTML=`<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet">${defs}${guides}${counts}${edgeSvg}${nodeSvg}${hiddenSvg(picked,visibleSet)}</svg>`;canvas.querySelectorAll('[data-adv-node]').forEach(g=>g.onclick=()=>selectAdvanced(g.dataset.advNode));renderStats(topoLive,picked.all.length,picked.visible,edges);renderFlows(topoLive);renderSelected(topoSelected);
}
