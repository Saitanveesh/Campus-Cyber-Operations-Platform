...
function advancedRender(live){
  ...
  const edgeSvg=edges.map(e=>{const a=pos[e.source],b=pos[e.target];if(!a||!b)return '';const recent=ageSeconds(e.last_seen)<8;const selected=topoSelected&&(e.source===topoSelected||e.target===topoSelected);const width=1+Math.min(4,Math.log10(1+Number(e.packets||0))/Math.max(1,Math.log10(1+maxPackets))*4);const opacity=selected?1:(topoMode==='flows'?.82:.48);const cls=`topo-edge ${recent?'recent':''} ${selected?'selected':''}`;const markerId=recent||selected?'topoArrowHot':'topoArrow';let label='';if(selected){const mx=(a.x+b.x)/2,my=(a.y+b.y)/2-4;label=`<text class="topo-edge-label" x="${mx}" y="${my}" text-anchor="middle">${h(edgeInfo(e))} · ${n(e.packets)}</text>`;}return `<line class="${cls}" x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}" style="stroke-width:${width};opacity:${opacity}" marker-end="url(#${markerId})"><title>${h(e.source)} → ${h(e.target)} | ${h(edgeInfo(e))} | ${n(e.packets)} packets | ${shortBytes(e.bytes)}</title></line>${label}`;}).join('');
