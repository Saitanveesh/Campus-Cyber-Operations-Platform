...
function handleBrowserFallback(d){
  ...
  for(const e of [...events].reverse()){
    ...
    if(e.source==='operations-watchdog'||e.kind==='INCIDENT'||e.kind==='NETWORK'||sev==='HIGH'||sev==='CRITICAL')browserSpeak(String(text));
  }
...
document.addEventListener('pointerdown',()=>{const v=(assistantData||{}).voice||{};if((!v.available||v.last_error)&&!v.last_spoken)browserSpeak('Welcome back, Sai Tanveesh. Live Operations Console is monitoring.',true);},{once:true});
...
def install_runtime_extensions(app: FastAPI) -> FastAPI:
    """Install live watchdog/assistant routes and inject the console extension."""
    ...
    async def console_extension(request: Request, call_next):
        if request.method == "GET" and request.url.path == "/":
            ...
            extended = html.replace("</body>", f"{UI_EXTENSION}\n</body>")
            return HTMLResponse(
                extended,
