...
def _is_local_address(ip: ipaddress._BaseAddress, network: dict[str, Any]) -> bool:
    for prefix in _network_values(network, "prefixes"):
        try:
            ...
            continue
    return str(ip) in set(_network_values(network, "ipv4") + _network_values(network, "ipv6"))


def _agent_for_ip(snapshot: dict[str, Any], target: str) -> dict[str, Any] | None:
    agents = snapshot.get("managed_agents")
    ...
    for agent in agents:
        ...
        for item in addresses:
            if isinstance(item, dict) and str(item.get("address") or "").strip() == target:
                return agent
    return None


def assess_target_truth(snapshot: dict[str, Any], target: str) -> dict[str, Any]:
    """Return only claims that are supported by current-session evidence.

    The function deliberately separates "observed" from "confirmed local asset" and
    "manageable". A syntactically valid IP with no packet/flow/asset/agent evidence is
    NOT_OBSERVED; it must never be treated as healthy, safe, or benign.
    """
    try:
    ...
    flow_count = sum(
        1
        for item in flows
        if str(item.get("src") or "") == target or str(item.get("dst") or "") == target
    )
    edge_count = sum(
        1
        for item in edges
        if str(item.get("source") or "") == target or str(item.get("target") or "") == target
    )
    ...
    observed = bool(asset or flow_count or edge_count or packet_count or alert_count or incident_count or agent)
